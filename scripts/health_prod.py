...
def main():
    ...
    parser.add_argument(
        "--binance", default="http://localhost:8080", help="Binance API url"
    )
...
class Health:
    ...
    def retrieve_data(self):
        ...
        for vault in self.thorchain_asgard_vaults:
            ...
            acct = self.binance_client.account(vault_addr)
            account = Account(acct["address"])
            if acct["balances"]:
                account.balances = []
                for b in acct["balances"]:
                    coin = Coin(
                        f"BNB.{b['symbol']}", float(b["free"]) * Coin.ONE
                    )
                    account.balances.append(coin)
